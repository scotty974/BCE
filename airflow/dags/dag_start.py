...
def dag_start():
    ...
    def get_entreprises_list():
        ...
        entreprises = [
            {"entity_number": "0475.960.588", "denomination": "RUDI ZEELMAEKERS, BEDRIJFSREVISOR"},
            {"entity_number": "0475.960.687", "denomination": "VOLLEY CLUB MARCHOIS"},
            {"entity_number": "0475.961.776", "denomination": "Rent-a-Priest - Emmaüs"},
            {"entity_number": "0200.065.765", "denomination": "Intergemeentelijke Vereniging Veneco"},
            {"entity_number": "0200.065.765", "denomination": "Veneco"},
            {"entity_number": "0200.068.636", "denomination": "Farys"},
            {"entity_number": "0200.171.970", "denomination": "Sanatorium-Hospitaal van Lemberge"},
            {"entity_number": "0200.245.711", "denomination": "Intercommunaal Sanatorium Denderoord"},
            {"entity_number": "0200.245.711", "denomination": "DENDEROORD"},
            {"entity_number": "0200.305.493", "denomination": "Intergemeentelijk Samenwerkingsverband voor ruimtelijke ordening en socio-economische expansie"},
            {"entity_number": "0200.305.493", "denomination": "SOLVA"},
            {"entity_number": "0200.362.210", "denomination": "in BW Association Intercommunale"},
            {"entity_number": "0200.362.210", "denomination": "in B.W."},
            {"entity_number": "0200.362.408", "denomination": "Intercommunale Sociale du Brabant wallon"},
            {"entity_number": "0200.362.408", "denomination": "I.S.B.W."},
            {"entity_number": "0200.420.410", "denomination": "Congregatie der Gasthuiszusters-Augustinessen van Diest"},
            {"entity_number": "0200.420.608", "denomination": "Congregatie der Gasthuiszusters van Poperinge"},
            {"entity_number": "0200.448.421", "denomination": "Liefdadige Congregatie der Gasthuiszusters"},
            {"entity_number": "0200.450.005", "denomination": "Liefdadige Congregatie der Zwarte Zusters van Brugge, Oostende en Menen (VL - Brugge)"},
            {"entity_number": "0200.762.878", "denomination": "VLOTTER"},
            {"entity_number": "0200.881.951", "denomination": "Intercommunale Maatschappij voor de Ruimtelijke Ordening en de Economisch- Sociale Expansie van het Arrondissement Halle-Vilvoorde"},
            {"entity_number": "0200.881.951", "denomination": "HAVILAND"},
            {"entity_number": "0200.882.050", "denomination": "Intercommunale maatschappij voor de Sanering van het Dal der Molenbeek en Pontbeek"},
            {"entity_number": "0200.882.050", "denomination": "VALLEE MOLENBEEK PONTBEEK DAL"},
            {"entity_number": "0201.105.843", "denomination": "I.D.E.A. S.C"},
            {"entity_number": "0201.105.843", "denomination": "I.D.E.A."},
            {"entity_number": "0201.107.526", "denomination": "ASSOCIATION INTERCOMMUNALE DU BOIS D'HAVRE"},
            {"entity_number": "0201.107.526", "denomination": "I.B.H."},
            {"entity_number": "0201.183.146", "denomination": "Société Intercommunale de l'Electricité de la Dendre et du Canton de Lens"},
            {"entity_number": "0201.183.146", "denomination": "I.D.E.L."},
            {"entity_number": "0201.310.731", "denomination": "NOORDLIMBURGSE MAATSCHAPPIJ VOOR DE OPRICHTING VAN EEN INDUSTRIEPARK IN NOORD-LIMBURG"},
            {"entity_number": "0201.310.731", "denomination": "NOLIMPARK"},
            {"entity_number": "0201.310.929", "denomination": "IGL"},
            {"entity_number": "0201.310.929", "denomination": "IGL"},
            {"entity_number": "0201.311.028", "denomination": "Intercommunale Maatschappij voor Ruimtelijke Ontwikkeling in Limburg"},
            {"entity_number": "0201.311.028", "denomination": "I.M.L."},
            {"entity_number": "0201.311.226", "denomination": "FLUVIUS"},
            {"entity_number": "0201.339.039", "denomination": "Provinciale Limburgse Intercommunale Gasmaatschappij"},
            {"entity_number": "0201.339.039", "denomination": "Pli Gas"},
            {"entity_number": "0201.400.011", "denomination": "SOCIETE INTERCOMMUNALE BEP - EXPANSION ECONOMIQUE"},
            {"entity_number": "0201.400.110", "denomination": "Association Intercommunale des Eaux du Condroz"},
            {"entity_number": "0201.400.110", "denomination": "A.I.E.C."},
            {"entity_number": "0201.400.209", "denomination": "Société intercommunale BEP-Environnement"},
            {"entity_number": "0201.456.924", "denomination": "Intercommunale de distribution d'Eau de Tihange et environs"},
            {"entity_number": "0201.456.924", "denomination": "TIHANGE"},
            {"entity_number": "0201.543.234", "denomination": "TIBI"},
            {"entity_number": "0201.552.538", "denomination": "Oeuvres Sociales Intercommunale"},
            {"entity_number": "0201.568.473", "denomination": "Union intercommunale pour l'Exécution des Travaux de Voutement et d'Amélioration des Ruisseaux de Lodelinsart"},
    ...
    def create_proxy_entreprise_pairs(proxy_list: list, entreprises_list: list):
        ...
        for i in range(nb_pairs):
            pairs.append({
                'proxy': proxy_list[i],
                'entreprise': entreprises_list[i]
            })
        
        print(f"🔗 {len(pairs)} paires (proxy, entreprise) créées")
        for i, pair in enumerate(pairs):
            print(f"   [{i+1}] {pair['proxy']} → {pair['entreprise']['entity_number']}")
    ...
    def trigger_scrapper(pair: dict, token: str):
        ...
        print(f"🚀 Trigger scraping: {entity_number} - {denomination}")
        print(f"   Proxy: {proxy}")

        dag_id = 'dag_exemple_scrappring'
        api_url = f"http://airflow-apiserver:8080/api/v2/dags/{dag_id}/dagRuns"

        # Nettoyer la denomination pour l'utiliser dans le run_id (enlever caractères spéciaux)
        safe_denomination = "".join(c for c in denomination if c.isalnum() or c in (' ', '-', '_')).strip()[:50]
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                return {'entity_number': entity_number, 'proxy': proxy, 'triggered': True}
            else:
    ...
    pairs = create_proxy_entreprise_pairs(proxy_list, entreprises_list)
