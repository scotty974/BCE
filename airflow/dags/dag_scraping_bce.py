...
def dag_scraping_bce():
    ...
    def recuperer_et_valider_proxies():
        ...
        try:
            ...
            logger.error(
                f"❌ Erreur lors de la récupération/validation des proxies: {str(e)}"
            )
            raise
    ...
    def scraper_entreprises(nb_proxies: int):
        ...
        def scrape_entreprise(entreprise: Dict) -> Dict:
            ...
            for attempt in range(max_retries):
                ...
                if proxy is None:
                    logger.error(
                        f"❌ Timeout - Aucun proxy disponible pour {entity_number}"
                    )
                    break
                ...
                try:
                    ...
                    logger.info(
                        f"🌐 Scraping {entity_number} via {proxy} (tentative {attempt + 1})"
                    )
                    ...
                    if response.status_code == 404:
                        logger.warning(
                            f"⚠️ 404 pour {entity_number} - Ignoré (ne pas stocker)"
                        )
                        return {
                            "status": "skipped",
                            "reason": "404",
            ...
            # Max retries atteint
            logger.error(
                f"❌ Échec du scraping de {entity_number} après {max_retries} tentatives"
            )
        ...
        with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_REQUESTS) as executor:
            futures = [
                executor.submit(scrape_entreprise, ent) for ent in entreprises_test
            ]
