...
class ProxyService:
    ...
    def recuperer_proxies_free_proxy_list(self) -> List[str]:
        ...
        try:
            ...
            logger.info(f"✅ {len(proxies)} proxies récupérés depuis free-proxy-list.net")
            return proxies
    ...
    def initialiser_proxies(self, proxies: List[str]):
        ...
        self.redis_client.delete('proxy_status', 'proxy_last_used', 'proxy_blocked_until')
    ...
    def charger_proxies_depuis_redis(self):
        """
        Charge la liste des proxies depuis Redis
        Utile quand le service est recréé dans une nouvelle tâche
        """
        all_proxies = self.redis_client.hkeys('proxy_status')
        self.proxies = [proxy.decode('utf-8') for proxy in all_proxies]
        logger.info(f"📋 {len(self.proxies)} proxies chargés depuis Redis")
    
    def get_available_proxy(self) -> Optional[str]:
        """
        Retourne un proxy disponible (pas utilisé depuis 20s, pas en cooldown)
        
        Returns:
            Proxy disponible ou None si aucun n'est disponible
        """
        ...
        for proxy in self.proxies:
            ...
            if status == b'blocked':
                ...
                if blocked_until and float(blocked_until) > current_time:
                    ...
                    # Cooldown terminé, réactiver
                    self.redis_client.hset('proxy_status', proxy, 'available')
            
            # Vérifier le dernier usage
    ...
    def attendre_proxy_disponible(self, timeout: int = 300) -> Optional[str]:
        ...
        while (time.time() - start_time) < timeout:
            ...
            time.sleep(5)
            stats = self.get_stats()
            logger.info(f"📊 Proxies - Dispo: {stats['available']}, Bloqués: {stats['blocked']}")
